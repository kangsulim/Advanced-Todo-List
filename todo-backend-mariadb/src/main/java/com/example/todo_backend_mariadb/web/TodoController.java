...
public class TodoController {
    ...
    @PostMapping
    public ResponseEntity<TodoResponseDto> createTodo(@RequestBody TodoCreateRequestDto requestDto, @AuthenticationPrincipal Jwt principal) {

        return ResponseEntity.status(HttpStatus.CREATED).body(todoService.createTodo(requestDto, principal.getClaimAsString("email")));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<TodoResponseDto> toggleTodo(@PathVariable Long id, @AuthenticationPrincipal Jwt principal) {
    ...
    public ResponseEntity<Void> deleteTodo(@PathVariable Long id, @AuthenticationPrincipal Jwt principal) {
        ...
        todoService.deleteDto(id, principal.getClaimAsString("email"));
